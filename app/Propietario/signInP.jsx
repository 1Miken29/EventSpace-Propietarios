...
export default function signInP() {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      router.push("/propietario/inicio");
    } else {
  ...
  return (
    <View className="bg-[#C3B6E3] w-full h-full items-center justify-center">
      <View className="bg-white rounded-[33px] w-96 px-4 items-center py-10">
        ...
        <TextInput
          className={`h-14 w-full border ${errors.email ? 'border-red-500 text-red-500' : 'border-[#C4C4C4] text-black'} rounded-xl p-3 font-outfit text-xl mb-2 ${formData.email ? 'text-black' : 'text-[#C4C4C4]'}`}
        ...
        <View className="relative w-full">
          <TextInput
            className={`h-14 w-full border ${errors.password ? 'border-red-500 text-red-500' : 'border-[#C4C4C4] text-black'} rounded-xl p-3 font-outfit text-xl mb-2 ${formData.password ? 'text-black' : 'text-[#C4C4C4]'}`}
        ...
        <View className="w-full">
          <Text className="font-outfit-medium text-xl text-left my-2">
            <Link href="/Propietario/recuperacion" className="font-outfit-medium">
